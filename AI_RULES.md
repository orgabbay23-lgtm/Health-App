...
* **2026-03-17: Active Days Target Accumulation (Clinical Math Fix)**
    * **Issue:** Weekly/Monthly period targets were multiplied by total days elapsed in the period (`periodDetails.dayKeys.length`). If a user didn't log meals on Sunday and Monday, Tuesday's weekly view showed a 3-day calorie budget (e.g., 6000 kcal) despite only having data for one day — artificially inflating targets and skewing progress bars and AI insights.
    * **Fix:** Replaced the naive `dayKeys.length` multiplier with an `activeDaysCount` that only counts: (1) Today (always counted — user needs a target for the current day), and (2) past days that have at least one logged meal. Minimum multiplier is 1. Applied in `Dashboard.tsx` `periodTargets` memo. Since `periodTargets` is passed as a prop to all consumers (progress bars, AI insight generator), the fix propagates automatically.
...
* **2026-03-21: WaterTracker Expandable Architecture & Aesthetic Overhaul**
    ...
    * **Collapsed State:** Icon + "���� �����" title + progress text ("X / Y ���") + percentage badge + animated gradient progress bar. Fits seamlessly between Macros and Safety Alerts.
...
* **2026-03-21: WaterTracker � Custom Amount (����� �����) Feature**
    * **Change:** Added a \"Custom Amount\" (����� �����) workflow to the `WaterTracker.tsx` component to allow precise logging.
    * **Implementation:** (1) Added `isCustomAddOpen` and `customAddValue` state for view toggling. (2) Replaced the static `QUICK_ADD_OPTIONS` list in the UI with an `<AnimatePresence mode=\"wait\">` that switches between the standard pill buttons and a \"Custom Input Panel\". (3) Added a \"����� �����\" button with a `Plus` icon as the final pill in the quick-add view. (4) The Custom Input Panel features a transparent `text-[16px]` numeric input (iOS zoom-safe), a green glassmorphism `Check` button, and a slate `X` cancel button. (5) Added \"Special Sizes\" (����� �������) pills for 180ml (��� ���), 330ml (���), and 600ml (����� �����) for common Israeli container sizes.
...
* **2026-03-21: Zustand Stale Closure Fix (logWater & removeLastWaterLog)**
    * **Issue:** dailyWaterAmount destructured at the top of Zustand action functions created a stale closure. When a 3 AM rollover reset the state to 0 inside etchTodayWater(), the original destructured variable still held yesterday's amount, resulting in optimistic updates erroneously compounding onto yesterday's total.
    * **Fix:**
        1. Moved the state retrieval (get().dailyWaterAmount) to occur strictly *after* the etchTodayWater() check completes.
        2. Ensured that emoveLastWaterLog calculates correct startOfDay and endOfDay using the getHydrationDayKey() boundary split (3:00 AM) to only fetch today's logs for deletion.
    * **Standard:** In Zustand actions that conditionally await other async state-mutating actions (like fetching new data on date rollover), always read the critical scalar values (e.g. get().dailyWaterAmount) *after* the wait finishes. Never destructure at the top of the function if an intermediate wait can invalidate the values.

* **2026-03-22: Unified AI Formatting & Toxicity-Aware Ranking**
    * **Issue:** (1) Classic Insight and Supplement recommendations had inconsistent formatting (hashes vs numbers), leading to broken rich-text rendering in the FormattedAIResponse component. (2) Supplement recommendations did not prioritize safety, potentially ranking high-toxicity minerals (like Iron) above safe water-soluble vitamins.
    * **Fix:**
        1. **Unified Formatting Standard:** Updated all AI system instructions (INSIGHT_SYSTEM_INSTRUCTION, CUSTOM_ANSWER_SYSTEM_INSTRUCTION, SUPPLEMENT_SYSTEM_INSTRUCTION, FOLLOWUP_SYSTEM_INSTRUCTION) to strictly use 1. Header, - Bullet, and **Bold** patterns. Explicitly FORBIDDEN the use of hashes (#), backticks, or other markdown headers.
        2. **Resilient Formatter:** Enhanced FormattedAIResponse in SmartInsightGenerator.tsx to be resilient to space variations after headers/bullets (e.g., 1.Text vs 1. Text) and ensured headers use shrink-0 to prevent layout collapse.
...
* **2026-03-22: Floating Period Selection Tabs (Mobile UI Repositioning)**
    * **Change:** Repositioned the Period Selection Tabs (����/�����/�����) from the top of the screen to a floating, fixed position at the bottom of the viewport on mobile.
    * **Implementation:**
...
* **2026-03-22: App-Wide 60/120fps Performance Pass**
    ...
    * **useCallback Stabilization:** 11 event handlers in `Dashboard.tsx` were recreated every render and prop-drilled to child trees. Wrapped in `useCallback` to maintain referential equality: `onSaveFavorite`, `scrollToTop`, `onHistoryIncrement`, `onHistoryDecrement`, `onSelectArchiveDay`, `onEditMeal`, `onOpenMealModal`, `onCloseMealModal`, `onOpenProfileModal`, `onCloseProfileModal`, `onCloseEditLog`.
    * **useMemo for Derived Data:** Memoized `keys`, `profileData`, and `nutritionData` in `SmartInsightGenerator.tsx`. Memoized `periodCaption` in `HomeScreen.tsx`. Extracted static Framer Motion variant objects (`containerVariants`, `itemVariants`) to module scope.
    * **React.memo() on Presentational Components:** `NutrientCard` (rendered up to 25x), `CompactNutrientCard` (3x), `MealTimelineItem` (N per meal), and `FormattedAIResponse` wrapped in `React.memo()` to skip re-renders when props haven't changed.
    * **GPU Hardware Acceleration:** Added `willChange` hints on: wave animation (`WaterTracker`), progress ring SVG + animated circle (`PrimaryNutrientCard`), nutrient progress bars (`NutrientCard`, `CompactNutrientCard`), floating nav (`BottomNavigation` — also `translateZ(0)`), and meal timeline cards (`MealTimeline`).
    * **Input Debounce:** `FoodTypeahead.tsx` suggestion scoring (iterating all daily logs + 10K food database) debounced by 120ms to prevent main-thread blocking during rapid typing.
    * **Mobile Bug Fixes (Post-Audit):**
        1. **`layout` prop removal:** Removed raw `<motion.div layout>` from `SmartInsightGenerator.tsx`, `PeriodBreakdown.tsx`, and `MealTimeline.tsx` (card wrapper + expand section). These caused iOS Safari viewport jumps when the keyboard resized the viewport, because Framer Motion's FLIP algorithm recalculated stale DOM metrics inside scroll containers. `AnimatePresence` height animations work correctly without `layout`.
        2. **`willChange` cleanup:** Removed persistent `willChange` from 6 one-shot animation elements (BottomNavigation, NutrientCard bars, CompactNutrientCard bars, PrimaryNutrientCard SVG + circle, MealTimeline cards). Replaced with `translateZ(0)` where GPU compositing is still needed. Only the WaterTracker infinite wave animation retains `willChange: "transform"`.
        3. **ModalShell scroll lock:** Changed the useEffect cleanup to re-query `document.querySelector('.ios-scroll-canvas')` at teardown time instead of using a stale closure reference. Prevents frozen scroll on rapid modal open/close.
        4. **iOS keyboard scroll timing:** Increased all `scrollIntoView` timeouts from 350ms to 450ms across 7 files (16 occurrences) to account for slower keyboard animations on low-end iOS devices.
        5. **RTL screen-transition horizontal glitch:** The outer `<div dir="rtl">` used `overflow-x-hidden`, which creates a scroll context. In RTL, the scroll origin is at the right edge. During `AnimatePresence mode="wait"` screen transitions, the brief content collapse between exit-complete and enter-start caused the browser to recalculate the scroll position, producing a visible horizontal shift. Fixed by changing `overflow-x-hidden` to `overflow-x-clip` (clips without creating a scroll context). Also simplified the screen exit animation from `{ opacity: 0, y: -15 }` to `{ opacity: 0 }` — the `y` offset during exit added unnecessary layout interaction in the RTL scroll context.
    * **Standard:** All Zustand consumers MUST use atomic selectors — never destructure the full store. All prop-drilled callbacks MUST be `useCallback`-wrapped. Frequently-rendered presentational components receiving primitive/stable props MUST use `React.memo()`. Only use `willChange` on continuously-running infinite animations; for one-shot animations, use `translateZ(0)` instead. Never use raw `<motion.div layout>` inside scroll containers — use `SafeLayoutMotion` or remove the `layout` prop if `AnimatePresence` height animation suffices. In RTL containers, use `overflow-x-clip` instead of `overflow-x-hidden` to prevent scroll-context reflow during AnimatePresence transitions.
